--- ### mlnvim Utilities
--
-- This module is automatically loaded by mlnvim on during it's initialization into global variable `mlnvim`
--
-- This module can also be manually loaded with `local mlnvim = require "core.utils"`
--
-- @module core.utils
-- @copyright 2022
...
mlnvim.install = mlnvim_installation or { home = stdpath("config") }
--- external mlnvim configuration folder
mlnvim.install.config = stdpath("config"):gsub("nvim$", "mlnvim")
vim.opt.rtp:append(mlnvim.install.config)
local supported_configs = { mlnvim.install.home, mlnvim.install.config }
...
mlnvim.user_settings = load_module_file("user.init")
--- default packer compilation location to be used in bootstrapping and packer setup call
mlnvim.default_compile_path = stdpath("data") .. "/packer_compiled.lua"
--- table of user created terminals
mlnvim.user_terminals = {}
...
-- @function mlnvim.func_or_extend
...
local function func_or_extend(overrides, default, extend)
	...
	if extend then
		...
		if type(overrides) == "table" then
			default = mlnvim.default_tbl(overrides, default)
...
function mlnvim.default_tbl(opts, default)
	opts = opts or {}
...
function mlnvim.conditional_func(func, condition, ...)
...
function mlnvim.trim_or_nil(str)
...
function mlnvim.pad_string(str, padding)
	padding = padding or {}
...
function mlnvim.initialize_icons()
	mlnvim.icons = mlnvim.user_plugin_opts("icons", require("core.icons.nerd_font"))
	mlnvim.text_icons = mlnvim.user_plugin_opts("text_icons", require("core.icons.text"))
end

--- Get an icon from `lspkind` if it is available and return it
-- @param kind the kind of icon in `lspkind` to retrieve
-- @return the icon
function mlnvim.get_icon(kind)
	local icon_pack = vim.g.icons_enabled and "icons" or "text_icons"
	if not mlnvim[icon_pack] then
		mlnvim.initialize_icons()
	end
	return mlnvim[icon_pack] and mlnvim[icon_pack][kind] or ""
end

--- Serve a notification with a title of mlnvim
...
function mlnvim.notify(msg, type, opts)
	vim.notify(msg, type, mlnvim.default_tbl(opts, { title = "mlnvim" }))
end
...
function mlnvim.confirm_prompt(messages)
	if messages then
		mlnvim.echo(messages)
	end
...
local function user_setting_table(module)
	...
	local settings = mlnvim.user_settings or {}
...
function mlnvim.initialize_packer()
	-- try loading packer
	...
	if not packer_avail then
		...
		mlnvim.echo({ { "Initializing Packer...\n\n" } })
	...
	if packer_avail then
		...
		local run_me, _ = loadfile(
			mlnvim.user_plugin_opts("plugins.packer", { compile_path = mlnvim.default_compile_path }).compile_path
		)
		...
		-- if there is no compiled file, prompt the user to run :PackerSync
			...
			mlnvim.echo({ { "Running " }, { ":PackerSync", "Title" } })
			require("packer").sync()
		end
	end
end
...
function mlnvim.vim_opts(options)
...
function mlnvim.user_plugin_opts(module, default, extend, prefix)
...
function mlnvim.url_opener()
	...
	-- if any other operating system notify the user that there is currently no support
		...
		mlnvim.notify("gx is not supported on this OS!", "error")
	end
end
...
function mlnvim.toggle_term_cmd(term_details)
	...
	if mlnvim.user_terminals[term_key] == nil then
		mlnvim.user_terminals[term_key] = require("toggleterm.terminal").Terminal:new(term_details)
	end
	-- toggle the terminal
	mlnvim.user_terminals[term_key]:toggle()
end

--- Add a source to cmp
-- @param source the cmp source string or table to add (see cmp documentation for source table format)
function mlnvim.add_cmp_source(source)
...
function mlnvim.get_user_cmp_source(source)
	...
	local priority = mlnvim.user_plugin_opts("cmp.source_priority", {
		nvim_lsp = 1000,
		luasnip = 750,
		buffer = 500,
		path = 250,
...
function mlnvim.add_user_cmp_source(source)
	mlnvim.add_cmp_source(mlnvim.get_user_cmp_source(source))
end
...
function mlnvim.which_key_register(mappings, opts)
...
function mlnvim.null_ls_providers(filetype)
	local registered = {}
	-- try to load null-ls
...
function mlnvim.null_ls_register(source)
	-- try to load null-ls
...
function mlnvim.null_ls_sources(filetype, method)
	local methods_avail, methods = pcall(require, "null-ls.methods")
	return methods_avail and mlnvim.null_ls_providers(filetype)[methods.internal[method]] or {}
end
...
function mlnvim.alpha_button(sc, txt)
...
function mlnvim.is_available(plugin)
...
function mlnvim.set_mappings(map_table, base)
...
function mlnvim.delete_url_match()
...
function mlnvim.set_url_match()
	mlnvim.delete_url_match()
	if vim.g.highlighturl_enabled then
		vim.fn.matchadd("HighlightURL", mlnvim.url_matcher, 15)
	end
end
...
function mlnvim.cmd(cmd, show_error)
